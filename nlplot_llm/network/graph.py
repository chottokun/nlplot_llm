...
def build_graph(nlplot_instance, stopwords: list = [], min_edge_frequency: int = 10) -> None:
    ...
    _detect_communities(nlplot_instance, nlplot_instance.G)
    return None
...
def _detect_communities(nlplot_instance, G: nx.Graph) -> None:
    ...
    if not G or not isinstance(G, nx.Graph):
        logging.error("Invalid graph provided to _detect_communities: Graph is empty or not a networkx.Graph instance.")
        raise ValueError("Invalid graph provided to _detect_communities: Graph is empty or not a networkx.Graph instance.")

    try:
        partition = community_louvain.best_partition(G)
    except Exception as e:
        logging.error(f"Community detection failed: {e}")
        # フォールバックとして全てのノードにcommunity=0を割り当てる
        nlplot_instance.node_df["community"] = 0
        return

    # partitionのキーとnode_dfの"word"列の不一致を検出
    partition_keys = set(partition.keys())
    node_words = set(nlplot_instance.node_df["word"])
    missing_in_partition = node_words - partition_keys
    if missing_in_partition:
        logging.warning(f"Some words in node_df are missing in partition keys: {missing_in_partition}")

    # mapで欠損はNaNになるため、欠損は0に置換しint型に変換
    nlplot_instance.node_df["community"] = nlplot_instance.node_df["word"].map(partition).fillna(0).astype(int)
