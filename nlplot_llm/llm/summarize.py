...
try:
    ...
    # Dummy classes for litellm and text splitters if not installed
...
def _chunk_text(
    ...
    **splitter_kwargs,
    ...
    if not LANGCHAIN_SPLITTERS_AVAILABLE:
        print(
            "Warning: Langchain text splitter components are not "
            "installed. Chunking will not be performed; returning "
    ...
    if strategy == "recursive_char":
        ...
        raise ValueError(
            ...
            "Supported strategies: 'recursive_char', 'character'."
        )
...
async def _summarize_text_single_async(
    ...
    original_text_for_df = (
        str(text_to_summarize) if pd.notna(text_to_summarize) else ""
    )
    summary_text = ""
    ...
    if not original_text_for_df.strip():
        ...
        _actual_chunk_prompt = (
            chunk_prompt_template_str_in or
            "Concisely summarize this piece of text: {text}"
        )
        ...
        if use_chunking and LANGCHAIN_SPLITTERS_AVAILABLE:
            temp_chunks_for_key = _chunk_text(
                original_text_for_df,
                chunk_size=chunk_size,
                chunk_overlap=chunk_overlap,
            )
        ...
        cache_key_tuple = (
            ...
            _actual_combine_prompt
            if use_chunking and num_chunks_for_key > 1
            else None,
            original_text_for_df,
            use_chunking,
            chunk_size if use_chunking else None,
            chunk_overlap if use_chunking else None,
        ...
        if current_use_cache and nlplot_instance.cache:
            try:
                ...
                print(
                    "Warning: Async cache get failed for summarize. "
                    f"Error: {e}"
                )
        ...
        if cached_result is not None:
            ...
            if "temperature" not in _final_completion_kwargs:
                _final_completion_kwargs["temperature"] = temperature
            ...
            if not use_chunking or not LANGCHAIN_SPLITTERS_AVAILABLE:
                try:
                    messages = [
                        {
                            "role": "user",
                            "content": direct_prompt_template_str.format(
                    ...
                    if use_chunking and not LANGCHAIN_SPLITTERS_AVAILABLE:
                        raw_llm_response_agg = (
                            "Chunking skipped (splitters unavailable). "
                            + raw_llm_response_agg
                        )
                except Exception:
                    summary_text = "error"
                    raw_llm_response_agg = "Async direct/fallback summary error"
            else:
                chunks = _chunk_text(
                    original_text_for_df,
                    chunk_size=chunk_size,
                ...
                if not chunks:
                    summary_text = ""
                    raw_llm_response_agg = (
                        "Text was empty or too short after attempting to "
                        "chunk."
                    )
                else:
                    ...
                    if "{text}" not in current_chunk_prompt_str:
                        ...
                        for chunk_item_text in chunks:
                            messages_chunk = [
                                {
                                    "role": "user",
                                    "content": current_chunk_prompt_str.format(
                                        text=chunk_item_text
                                    ),
                                }
                            ]
                            chunk_tasks.append(
                        ...
                        try:
                            ...
                            for (
                                i_chunk,
                                response_chunk_or_exc,
                            ) in enumerate(chunk_responses):
                                if isinstance(
                                    response_chunk_or_exc, Exception
                                ):
                                    chunk_summaries_list.append(
                                        f"[Error in chunk {i_chunk + 1}]"
                                    )
                                    raw_chunk_outputs_list.append(
                                        f"Chunk {i_chunk + 1} Error: "
                                        f"{str(response_chunk_or_exc)}"
                                    )
                                elif (
                                    response_chunk_or_exc.choices and
                                    response_chunk_or_exc.choices[0].message
                                ):
                                    chunk_summaries_list.append(
                                        response_chunk_or_exc.choices[
                                            0
                                        ].message.content.strip()
                                    )
                                    raw_chunk_outputs_list.append(
                                        f"Chunk {i_chunk + 1} Raw: "
                                        f"{str(response_chunk_or_exc)}"
                                    )
                                else:
                                    chunk_summaries_list.append("")
                                    raw_chunk_outputs_list.append(
                                        f"Chunk {i_chunk + 1} Raw (empty): "
                                        f"{str(response_chunk_or_exc)}"
                                    )
                        except Exception as e_gather_chunks:
                            summary_text = "error"
                            raw_llm_response_agg = (
                                "Error gathering chunk summaries: "
                                f"{e_gather_chunks}"
                            )
                    ...
                    if not summary_text.startswith("error"):
                        ...
                        if _actual_combine_prompt and len(chunks) > 1:
                            if "{text}" not in _actual_combine_prompt:
                                ...
                                raw_llm_response_agg += (
                                    "\n---\nWarning: Combine prompt invalid, "
                                    "using intermediate."
                                )
                            else:
                                try:
                                    messages_combine = [
                                        {
                                            "role": "user",
                                            "content": _actual_combine_prompt.format(
                                    ...
                                    summary_text = (
                                        response_combine.choices[
                                            0
                                        ].message.content.strip()
                                        if response_combine.choices and
                                        response_combine.choices[0].message
                                        else ""
                                    )
                                    raw_llm_response_agg += (
                                        "\n---\nFinal Combined Summary Raw: "
                                        f"{str(response_combine)}"
                                    )
                                except Exception as e_combine:
                                    summary_text = (
                                        "[Error combining: "
                                        f"{intermediate_summary[:100]}...]"
                                    )
                                    raw_llm_response_agg += (
            ...
            if (
                current_use_cache and
                nlplot_instance.cache and
                not summary_text.startswith("error_") and
                not summary_text.startswith("[Error")
            ):
                try:
                    ...
                    print(
                        "Warning: Async cache set failed for summarize. "
                        f"Error: {e}"
                    )
...
async def summarize_text_llm_async(
    ...
    **litellm_kwargs,
    ...
    if "temperature" not in kw_main:
        kw_main["temperature"] = temperature
    if max_length and "max_tokens" not in kw_main:
        kw_main["max_tokens"] = max_length

    direct_prompt = (
        prompt_template_str or
    ...
    if "{text}" not in direct_prompt:
        return pd.DataFrame(
            [
                {
                    "original_text": str(t),
                    "summary": "error_prompt_direct",
                    "raw_llm_output": "Direct prompt error",
                }
    ...
    if (
        use_chunking and
        chunk_prompt_template_str and
        "{text}" not in chunk_prompt_template_str
    ):
        return pd.DataFrame(
            [
                {
                    "original_text": str(t),
                    "summary": "error_prompt_chunk",
                    "raw_llm_output": "Chunk prompt error",
                }
                for t in text_series
            ],
            columns=cols,
    ...
    async def wrapper(text):
        if sem:
            async with sem:
                return await _summarize_text_single_async(
                    nlplot_instance,
                    text,
                    model,
                    direct_prompt,
    ...
    for txt_in in text_series:
        tasks.append(wrapper(txt_in))
            ...
            proc_res.append(
                {
                    ...
                    "summary": "error",
                    "raw_llm_output": str(r_exc),
                }
            )
        else:
            proc_res.append(r_exc)
...
def summarize_text_llm(
    ...
    **litellm_kwargs,
    ...
    if not LITELLM_AVAILABLE:
        print(
            "Warning: LiteLLM is not installed. LLM-based summarization "
            "is not available."
        )
    ...
    final_direct_prompt_template_str = (
        prompt_template_str or "Please summarize the following text concisely: {text}"
    )
    if "{text}" not in final_direct_prompt_template_str:
        print(
            "Error: Direct summarization prompt template must include "
            "'{text}' placeholder."
        )
        return pd.DataFrame(
            [
                {
                    "original_text": str(txt) if pd.notna(txt) else "",
                    "summary": "error_prompt_direct",
                    "raw_llm_output": "Direct prompt error",
                }
    ...
    for text_input in text_series:
        ...
        if not original_text_for_df.strip():
            ...
            cacheable_kwargs_items = sorted(
                (k, v)
                for k, v in _litellm_kwargs.items()
                if k in ("temperature", "max_tokens", "top_p")
            )
            _effective_chunk_prompt = (
                chunk_prompt_template_str or
                "Concisely summarize this piece of text: {text}"
            )
            ...
            if cached_result is not None:
                ...
                if not use_chunking:
                    try:
                        messages = [
                            {
                                "role": "user",
                                "content": final_direct_prompt_template_str.format(
                                    text=original_text_for_df
                                ),
                            }
                        ]
                        ...
                        print(
                            "Error during direct summarization for text "
                    ...
                    if not LANGCHAIN_SPLITTERS_AVAILABLE:
                        print(
                            "Warning: Langchain text splitters not "
                            "available. Attempting direct summarization."
                        )
                        try:
                            messages = [
                                {
                            ...
                            raw_llm_response_agg = (
                                "Chunking skipped. Direct summary raw: "
                                f"{str(response)}"
                            )
                        except Exception as e:
                            print(
                                "Error during fallback direct "
                                f"summarization: {e}"
                            )
                            summary_text = "error"
                            raw_llm_response_agg = (
                                "Fallback direct summarization error: {e}"
                            )
                    else:
                        chunks = _chunk_text(
                            original_text_for_df,
                        ...
                        if not chunks:
                            ...
                            if "{text}" not in current_chunk_prompt:
                                summary_text = "error_prompt_chunk"
                                raw_llm_response_agg = "Chunk prompt error"
                                results.append(
                                    {
                                        "original_text": original_text_for_df,
                                        "summary": summary_text,
                            ...
                            for i, chunk_item_text in enumerate(chunks):
                                try:
                                    messages_chunk = [
                                        {
                                            "role": "user",
                                            "content": current_chunk_prompt.format(
                                                text=chunk_item_text
                                            ),
                                        }
                                    ]
                                    response_chunk = litellm.completion(
                                        model=model,
                                        messages=messages_chunk,
                                        **_litellm_kwargs,
                                    )
                                    chunk_summary = (
                                        response_chunk.choices[
                                            0
                                        ].message.content.strip()
                                    ...
                                    raw_chunk_outputs_list.append(
                                        f"Chunk {i + 1}/{len(chunks)} Raw: "
                                        f"{str(response_chunk)}"
                                    )
                                ...
                                if "{text}" not in _effective_combine_prompt:
                                    ...
                                    try:
                                        messages_combine = [
                                            {
                                                "role": "user",
                                                "content": _effective_combine_prompt.format(
                                                    text=intermediate_summary
                                                ),
                                            }
                                        ]
                                        ...
                                        summary_text = (
                                            ...
                                            if response_combine.choices
                                            and response_combine.choices[
                                                0
                                            ].message
                                            else ""
                                        )
                ...
                if (
                    current_use_cache_for_call
                    and nlplot_instance.cache
                    and not summary_text.startswith("error_")
                    and not summary_text.startswith("[Err")
                ):
                    try:
                        nlplot_instance.cache.set(
                            cache_key_tuple,
