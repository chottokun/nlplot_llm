...
try:
    ...
    class JanomeTokenizer:
        def __init__(self, wakati=False):
            pass
...
def get_tfidf_top_features(
    ...
    if not SKLEARN_AVAILABLE:
        print(
            "Error: scikit-learn is not installed. TF-IDF functionality "
        ...
        return pd.DataFrame(
            columns=(
                ...
                else ["document_id", "word", "tfidf_score"]
            )
        )

    if not isinstance(text_series, pd.Series):
        print(
            "Warning: text_series is not a pandas Series. "
    ...
    if (
        language == "japanese"
        and use_janome_tokenizer_for_japanese
        and JANOME_AVAILABLE
    ):
        try:
            janome_tokenizer_for_tfidf = JanomeTokenizer(wakati=True)
        except Exception as e:
            print(
                "Warning: Failed to initialize Janome Tokenizer for "
                f"TF-IDF (wakati=True): {e}. Will fallback."
            )
    ...
    for i, text_content in enumerate(text_series):
        ...
        if janome_tokenizer_for_tfidf:
            try:
                tokens = list(
                    janome_tokenizer_for_tfidf.tokenize(text_content)
                )
                processed_corpus.append(" ".join(tokens))
            except Exception as e_tok:
                print(
                    "Error during Janome tokenization for document "
                    f"{i}: {e_tok}. Using raw text."
                )
    ...
    if (
        hasattr(nlplot_instance, "default_stopwords")
        and nlplot_instance.default_stopwords
    ):
    ...
    if custom_stopwords:
        if isinstance(custom_stopwords, list):
            ...
            print(
                "Warning: custom_stopwords should be a list of strings. "
                "It will be ignored."
            )
    ...
    try:
        if not any(processed_corpus):
            print(
                "Warning: Corpus for TF-IDF is empty after "
                "processing. Returning empty DataFrame."
            )
            return pd.DataFrame(
                columns=(
                    ["word", "tfidf_score"]
    ...
    if return_type == "overall":
        ...
        sum_tfidf_array = (
            sum_tfidf.A1 if hasattr(sum_tfidf, "A1") else sum_tfidf.flatten()
        )
        feature_scores = list(zip(feature_names, sum_tfidf_array))
        sorted_features = sorted(
            feature_scores, key=lambda x: x[1], reverse=True
        )
        ...
        if tfidf_matrix.shape[1] == 0:
            return pd.DataFrame(
                columns=["document_id", "word", "tfidf_score"]
            )
        results_data = []
        for i in range(tfidf_matrix.shape[0]):
            ...
            doc_feature_scores = [
                (feature_names[idx], doc_vector[0, idx])
                for idx in non_zero_indices
            ]
            sorted_doc_features = sorted(
                doc_feature_scores, key=lambda x: x[1], reverse=True
            )
            top_n_doc = sorted_doc_features[:n_features]
            doc_id = (
                text_series.index[i]
                if text_series.index.is_unique
                and not isinstance(text_series.index, pd.RangeIndex)
                else i
            )
        ...
        return pd.DataFrame(
            results_data, columns=["document_id", "word", "tfidf_score"]
        )
    else:
...
def get_kwic_results(
    ...
    if not isinstance(text_series, pd.Series) or not isinstance(
        keyword, str
    ):
        print(
            "Warning: Invalid input types for KWIC. text_series must be a "
            "Series and keyword a string."
        )
        return []
    if text_series.empty or not keyword.strip():
        print(
            "Warning: text_series is empty or keyword is blank. "
            "Returning empty list for KWIC."
        )
        return []
    ...
    if (
        ...
        try:
            ...
            print(
                "Warning: Failed to initialize Janome Tokenizer for KWIC "
                f"(wakati=True): {e}. Will fallback to space splitting."
            )
    ...
    for i, text_content in enumerate(text_series):
        ...
        if janome_tokenizer_for_kwic:
            try:
                tokens = list(
                    janome_tokenizer_for_kwic.tokenize(text_content)
                )
                all_tokenized_documents.append(tokens)
            except Exception as e_tok:
                print(
                    "Error during Janome tokenization for KWIC, document "
                    f"{i}: {e_tok}. Using space splitting as fallback."
                )
